...
if nav_option == "🏠 Home Screen":
    st.title("🍕 Cheesy Delights - Home Screen")
    st.write("Overview of current items and live stock:")
    ...
    if os.path.exists(INVENTORY_FILE):
        ...
        st.info("No saved inventory data found yet. Go to 'All Inventory & Daily Entry' to save records.")
        ...
        if not demand_df.empty:
            ...
            with w_col1:
                if st.session_state.brother_1_phone:
                    wa_link_1 = f"https://wa.me/{st.session_state.brother_1_phone}?text={encoded_text}"
            ...
            with w_col2:
                if st.session_state.brother_2_phone:
                    wa_link_2 = f"https://wa.me/{st.session_state.brother_2_phone}?text={encoded_text}"
...
# ==========================================
    ...
    default_expenses = pd.DataFrame([{"Expense Reason": "Utility / Bills", "Amount": 1500.0}, {"Expense Reason": "Raw Material Cash", "Amount": 2000.0}])
...
# ==========================================
    ...
    st.write("Analyze business revenue and profit over months and years based on saved records:")
    
    if os.path.exists(INVENTORY_FILE):
        inv_records = pd.read_csv(INVENTORY_FILE)
        prices_df = st.session_state.prices_data
        
        merged_rep = pd.merge(inv_records, prices_df, on="Item Name", how="left").fillna(0)
